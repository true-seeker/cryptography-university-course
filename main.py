...
class StribogHash:
    ...
    def __init__(self, message):
        ...
        if isinstance(message, str):
            for i in message:
                self.message_list.append(ord(i))
        elif isinstance(message, int):
            hexed = hex(message)[2:]
            for i in range(0, len(hexed), 2):
                self.message_list.append(int(hexed[i] + hexed[i + 1], 16))

    def hash(self):
        messages = list_division(self.message_list, HASH_LENGTH)
        messages.reverse()

        N = list(0 for _ in range(HASH_LENGTH))
...
def params_info(function_to_decorate):
    def wrapper(*args):
        # print()
        # print(function_to_decorate.__name__)
        # for i in args:
        #     pretty_print_hex(i)
        ans = function_to_decorate(*args)
        # pretty_print_hex(ans)
        # print()
...
def list_division(lst, sz):
    result = []
    b = len(lst) % sz
    if b != 0:
        result.append(lst[:b])
        lst = lst[b:]
    result += [lst[i:i + sz] for i in range(0, len(lst), sz)]
    return result


# @params_info
def hash_get_key(K, i):
    K = x_transformation(K, StribogHash.C[i])
    K = s_transformation(K)
    K = p_transformation(K)
    K = l_transformation(K)
    return K
...
@params_info
def s_transformation(state):
    assert len(state) == HASH_LENGTH
...
@params_info
def g_transformation(N, h, m):
    K = x_transformation(h, N)
    K = s_transformation(K)
    K = p_transformation(K)
    K = l_transformation(K)
...
def get_bytes(a):
    ...
    if len(hexed) % 2 == 1:
        hexed = '0' + hexed
    for i in range(0, len(hexed), 2):
        result.append(int(hexed[i] + hexed[i + 1], 16))
    if len(result) == 7:
        result = [0, ] + result
    return result
